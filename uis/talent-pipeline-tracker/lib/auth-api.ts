...
export async function forgotPassword(email: string): Promise<string> {
	const response = await publicAuthRequest<DetailResponse>("/auth/forgot-password", { email });
	return response.detail;
}

export async function resetPassword(token: string, newPassword: string): Promise<string> {
	const response = await publicAuthRequest<DetailResponse>("/auth/reset-password", {
		token,
...
export async function changePassword(currentPassword: string, newPassword: string): Promise<string> {
	const response = await protectedAuthRequest<DetailResponse>("/auth/change-password", {
		method: "POST",
